...
public class NFCUtil {
    ...
    public NFCUtil(Activity activity) {
        mNfcAdapter = NfcCheck(activity, true);
        NfcInit(activity);
    }
     ...
     * 构造函数，用于初始化nfc
     */
    public NFCUtil(Activity activity, boolean skip) {
        mNfcAdapter = NfcCheck(activity, skip);
        NfcInit(activity);
    }
     ...
     * 检查NFC是否打开
     */
    public static NfcAdapter NfcCheck(Activity activity, boolean skip) {
        NfcAdapter mNfcAdapter = NfcAdapter.getDefaultAdapter(activity);
        if (mNfcAdapter == null) {
            return null;
        } else {
            if (!mNfcAdapter.isEnabled() && skip) {
